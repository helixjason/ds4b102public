...
actionButton(inputId = "reset", label = "Reset", icon = icon("sync"), status = "warning")
...
processed_data_filtered_tbl  <-  eventReactive(eventExpr =  input$apply, valueExpr = {
  processed_data_tbl %>%
...
processed_data_filtered_tbl() %>%
  summarize(
    ...
    wise_metric = ((sum(str_detect(category_1, "Mountain"))) / (sum(str_detect(category_1, "Road")) + .00001)) %>% round(1)
  ) %>%
...
renderValueBox({
  ...
  valueBox(value = summary_values_tbl()$wise_metric, 
           caption = "Mountain to Road Ratio", 
           icon = "fa-star-half", 
           color = "info")
})
```
...
output$plotly_2 <- renderPlotly({
  ...
  g <- time_plot_tbl() %>%
    ...
    geom_point(aes(text = label_text), color = "#2c3e50", size = .1) +
