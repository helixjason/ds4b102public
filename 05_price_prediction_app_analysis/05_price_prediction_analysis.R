...
write_rds(model_xgboost, path = "00_model/model_xgboost.rds")
...
bikes_tbl %>% separate_bike_description(keep_description_column = TRUE, append = FALSE)
...
# g <- bind_bike_prediction(bikes_tbl, new_bike_tbl) %>%
#     
#     mutate(category_2 = fct_reorder(category_2, price)) %>%
#     mutate(label_text = str_glue("Price: {scales::dollar(price, accuracy = 1)}
#                                  Model: {model}
...
bind_bike_prediction(bikes_tbl, new_bike_tbl) %>% plot_bike_prediction(interactive =  TRUE)
